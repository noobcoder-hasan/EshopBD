...
<body>
    ...
    <div class="main-content">
        <div class="content-card">
            <h1>Order Details <span class="text-muted">#{{ $order->id }}</span></h1>
            ...
            <h2><i class="fas fa-shopping-cart me-2"></i>Order Items</h2>
            <ul class="order-items">
                @foreach($order->orderItems as $orderItem)
                    <li>
                        <span class="item-name">{{ $orderItem->product_name }}</span>
                        <div>
                            <span>{{ $orderItem->quantity }} x ${{ number_format($orderItem->price, 2) }}</span>
                            <span class="item-price ms-3">${{ number_format($orderItem->quantity * $orderItem->price, 2) }}</span>
                        </div>
                    </li>
                @endforeach
            </ul>
            ...
            <div class="shipping-status">
                <h2 class="mb-4"><i class="fas fa-truck me-2"></i>Shipping Status</h2>
                <form action="{{ route('admin.updateShippingStatus', $order->id) }}" method="POST" class="d-flex align-items-center">
                    @csrf
                    @method('PUT')
